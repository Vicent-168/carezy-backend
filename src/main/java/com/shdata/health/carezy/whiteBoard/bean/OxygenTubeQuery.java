 ...
 * 氧气管
 */
@Component("OxygenTubeQuery")
public class OxygenTubeQuery extends BaseQuery {
    @Resource
    private FormsIndexMapper formsIndexMapper;
    @Override
    public Map<String, String> queryXmzs(String yljgid, Date rq, List<String> wardIds) {
        List<RegistInformationVo> list = formsIndexMapper.singleDuctStatistics(yljgid, wardIds, Constants.DGZG_FLDM,Constants.YQG);
        return list.stream()
