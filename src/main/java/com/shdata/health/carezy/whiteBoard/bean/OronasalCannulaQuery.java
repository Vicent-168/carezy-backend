 ...
 * 口鼻插管
 */
@Component("oronasalCannulaQuery")
public class OronasalCannulaQuery extends BaseQuery {
    @Resource
    private FormsIndexMapper formsIndexMapper;
    @Override
    public Map<String, String> queryXmzs(String yljgid, Date rq, List<String> wardIds) {
        List<RegistInformationVo> list = formsIndexMapper.singleDuctStatistics(yljgid, wardIds, Constants.DGZG_FLDM,Constants.KBCG);
        return list.stream()
