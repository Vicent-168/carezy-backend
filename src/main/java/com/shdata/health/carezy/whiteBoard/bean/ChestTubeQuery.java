 ...
 * 胸管
 */
@Component("chestTubeQuery")
public class ChestTubeQuery extends BaseQuery {
    @Resource
    private FormsIndexMapper formsIndexMapper;
    @Override
    public Map<String, String> queryXmzs(String yljgid, Date rq, List<String> wardIds) {
        List<RegistInformationVo> list = formsIndexMapper.singleDuctStatistics(yljgid, wardIds, Constants.DGZG_FLDM,Constants.XG);
        return list.stream()
