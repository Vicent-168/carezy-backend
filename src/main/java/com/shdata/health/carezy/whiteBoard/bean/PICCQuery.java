 ...
 * PICC 导管
 */
@Component(value = "PICCQuery")
public class PICCQuery extends BaseQuery{
    @Resource
    private FormsIndexMapper formsIndexMapper;
    @Override
    public Map<String, String> queryXmzs(String yljgid, Date rq, List<String> wardIds) {
        List<RegistInformationVo> list = formsIndexMapper.singleDuctStatistics(yljgid, wardIds, Constants.DGZG_FLDM,Constants.PICCG);
        return list.stream()
