 ...
 * 动静脉插管
 */
@Component("arteriovenousIntubationQuery")
public class ArteriovenousIntubationQuery extends BaseQuery {
    @Resource
    private FormsIndexMapper formsIndexMapper;
    @Override
    public Map<String, String> queryXmzs(String yljgid, Date rq, List<String> wardIds) {
        List<RegistInformationVo> list = formsIndexMapper.singleDuctStatistics(yljgid, wardIds, Constants.DGZG_FLDM,Constants.DJMCG);
        return list.stream()
