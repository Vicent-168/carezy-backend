 ...
 * 失禁性皮炎
 */
@Component("IncontinenceRiskQuery")
public class IncontinenceRiskQuery extends BaseQuery{
    @Resource
    ...
    public Map<String, String> queryXmzs(String yljgid, Date rq, List<String> wardIds) {
        String wsid = formsIndexService.getWsidByylgjAndwsbm2(Constants.SJXPYHYBPF_WSBM, yljgid);
        if (wsid == null){
            return new HashMap<>();
        }
        List<RegistInformationVo> list = registInformationMapper.singleRiskSummary(yljgid, wardIds, wsid);
        // 使用Stream API进行分组和转换
        return list.stream()
